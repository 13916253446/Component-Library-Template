...
function vueWarpper() {
  ...
  if (isDev) {
    ...
    fileName = 'yhy-mobile-dev.css'
  } else if (isProduction) {
    if (isSpecial) {
      fileName = 'yhy-mobile.variable.css'
    } else {
      fileName = 'yhy-mobile.css'
    }
    distDir = LIB_DIR
  } else if (isTest) {
    distDir = TEST_OUTPUT_DIR
    fileName = 'yhy-mobile-test.css'
  }
  return vuePlugin({
...
const rollupPlugin = [
  ...
  ...(conditionHelper(!isDev, [
    aliasPlugin({
      ...
      'yhy-mobile/components': resolve('components'),
      'yhy-mobile/lib': resolve('lib'),
      'yhy-mobile': resolve('lib/mand-mobile.esm.js'),
